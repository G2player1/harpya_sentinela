...
@Service
public class GetLastEmployeeHealthDataListUsecase {

    private final IHealthDataRepository healthDataRepository;

    @Autowired
    public GetLastEmployeeHealthDataListUsecase(IHealthDataRepository healthDataRepository){
        this.healthDataRepository = healthDataRepository;
    }

    public List<HealthData> execute(){
        List<HealthData> healthDataList;
        try {
            return healthDataRepository.getAllLastHealthDataByEmployee();
        } catch (EntityNotFoundException | NoResultException e) {
            throw new CannotGetEntityDataException(400, ErrorEnum.LBDQ,"can get list of all employees last health data");
        }
    }
}
